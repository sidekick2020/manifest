...
export async function feedFromBack4App(config, state, skips, batch = {}) {
  const userLimit = batch.userLimit ?? 5;
  const postLimit = batch.postLimit ?? 8;
  const commentLimit = batch.commentLimit ?? 10;
  ...
  const fetches = [
    b4a(config.appId, config.restKey, PARSE_CLASS_USER, {
      keys: 'objectId,username,sobrietyDate,createdAt,proPic,updatedAt',
      limit: userLimit,
      skip: skips.userSkip,
  ...
  let added = 0;
  for (const u of nU) {
    // Handle proPic - Parse File objects have __type: "File", name, and url properties
    let proPicUrl = null;
    if (typeof u.proPic === 'string') {
      proPicUrl = u.proPic;
    } else if (u.proPic && typeof u.proPic === 'object' && u.proPic.url) {
      proPicUrl = u.proPic.url;
    }
