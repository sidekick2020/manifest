 ...
 * Tracks exhaustion per entity type to skip unnecessary API calls.
 *
 * @param {typeof DEFAULT_CONFIG} config
 * @param {{ members: Map, posts: Map, comments: Map }} state - mutable
 * @param {{ userSkip: number, postSkip: number, commentSkip: number, exhausted?: Object }} skips - mutable
 * @param {{ userLimit?: number, postLimit?: number, commentLimit?: number }} batch
 * @returns {Promise<{ added: number, epochDate: Date | null, allExhausted: boolean }>}
 */
export async function feedFromBack4App(config, state, skips, batch = {}) {
  ...
  // Track exhausted entity types to skip unnecessary API calls
  if (!skips.exhausted) skips.exhausted = {};
  const ex = skips.exhausted;

  const fetches = [];
  const fetchMap = []; // tracks which index = which entity type
  ...
  if (!ex.comments) {
    ...
    fetchMap.push('comments');
  }

  if (sdcLimit > 0 && !ex.sdc) {
    fetches.push(
  ...
  // All entity types exhausted — no API calls needed
  if (fetches.length === 0) {
    return { added: 0, epochDate: null, memberCount: state.members.size, allExhausted: true };
  }

  const results = await Promise.all(fetches);

  // Unpack results by entity type
  let nU = [], nP = [], nC = [], nSDC = [];
  for (let i = 0; i < fetchMap.length; i++) {
    const type = fetchMap[i];
    const data = results[i];
    if (type === 'users') {
      nU = data;
      if (data.length < userLimit) ex.users = true;
    } else if (type === 'posts') {
      nP = data;
      if (data.length < postLimit) ex.posts = true;
    } else if (type === 'comments') {
      nC = data;
      if (data.length < commentLimit) ex.comments = true;
    } else if (type === 'sdc') {
      nSDC = data;
      if (data.length < sdcLimit) ex.sdc = true;
    }
  }
  ...
  const allExhausted = !!ex.users && !!ex.posts && !!ex.comments && (sdcLimit === 0 || !!ex.sdc);
  return { added, epochDate, memberCount: state.members.size, allExhausted };
}
