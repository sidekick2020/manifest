 ...
 * @param {number} prevTierIndex — previous tier index for hysteresis (caller manages state)
 * @returns {{ name: string, cellSize: number, blendFactor: number, tierIndex: number }}
 */
export function getLODTier(d, prevTierIndex = 0) {
  let tierIndex = prevTierIndex;
  ...
  return { name: tier.name, cellSize: tier.cellSize, blendFactor, tierIndex };
}

/**
 * Backward-compatible: get LOD level name (stateless — no hysteresis).
 */
...
/**
 * Backward-compatible: get cell size for aggregation (stateless — no hysteresis).
 */
...
/**
 * @deprecated Use getLODTier(d, prevTierIndex) instead. Kept for backward compat.
 */
export function resetLODState() {
  // No-op: hysteresis state is now managed by the caller.
}
