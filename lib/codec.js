...
export function computeMemberSeed(mid, ms, state) {
  ...
  const sobrDays = m.sobriety
    ...
    : 0;
  s += ':p' + pc + ':c' + cc + ':sob' + sobrDays + ':n[' + Object.keys(connected).sort().join(',') + ']:s' + ms;
  const encoded = { postCount: pc, commentCount: cc, connections: Object.keys(connected).length, sobrietyDays: sobrDays };
...
export function computeNeighborhoodsFromStructure(ms, state, clusterStructure) {
  const { clusters } = clusterStructure;
  const nhs = new Map();
  for (let i = 0; i < clusters.length; i++) {
    const seeds = clusters[i].sort().map((id) => computeMemberSeed(id, ms, state));
    nhs.set('nh_' + i, {
...
export function encodeWithSeed(seed, state, params = DEFAULT_PARAMS, predictions = null, precomputedNhs = null) {
  const { members } = state;
  ...
  members.forEach((_, mid) => {
    ...
    const mass = massByMember.get(mid) ?? computeMass(mid, state, params);
    let nhC = vec3(), nhS = 1;
    nhs.forEach((nh) => {
...
export function scoreCohesion(positions, state, params = DEFAULT_PARAMS) {
  ...
  // Accumulate comment count per unique pair (normalized key: smaller id first).
  // More comments from one user to another = more gravity (stronger pull toward each other).
  const pairs = {};
  comments.forEach((c) => {
...
export function evolve(state, params = DEFAULT_PARAMS, predictions = null) {
  ...
  for (let i = 0; i < vc; i++) {
    ...
    const nhs = computeNeighborhoodsFromStructure(seed, state, clusterStructure);
    const r = encodeWithSeed(seed, state, params, predictions, nhs);
 ...
 * Regions list merges region + state (e.g. Arkansas in state appears in regions).
 */
export function getLocationFilterOptions() {
  const regionAndState = [...locationOptions.regions, ...locationOptions.states];
  return {
    countries: [...locationOptions.countries].sort(),
    regions: [...new Set(regionAndState)].sort(),
