...
export const DEFAULT_PARAMS = {
  ...
  dynamicGravityRef: 2000,    // reference member count where scale = 1.0 (matches temporal window)
  dynamicGravityMin: 0.08,    // minimum scale factor (very few members → very tight)
  dynamicGravityMax: 1.0,     // maximum scale factor (never spread wider than base layout)
};

/**
 * Compute a dynamic gravity scale factor based on the number of visible members.
 * Fewer members → smaller scale → stars pulled closer together.
 * More members → larger scale → stars spread further apart.
 *
 * Uses sqrt scaling so the relationship is sub-linear:
 *   10 members → ~0.08,  50 → ~0.16,  200 → ~0.32,  500 → ~0.50,  2000 → 1.0
 *
 * @param {number} visibleCount - Number of currently visible/rendered members
 * @param {object} [params] - Codec params (uses dynamicGravityRef/Min/Max)
 * @returns {number} Scale factor to apply to positions (relative to centroid)
 */
export function computeDynamicScale(visibleCount, params = DEFAULT_PARAMS) {
  const ref = params.dynamicGravityRef ?? 500;
  const min = params.dynamicGravityMin ?? 0.15;
  const max = params.dynamicGravityMax ?? 1.5;
  if (visibleCount <= 0) return min;
  const raw = Math.sqrt(visibleCount / ref);
...
export function evolve(state, params = DEFAULT_PARAMS, predictions = null) {
  ...
  for (let i = 0; i < vc; i++) {
    ...
    const nhs = computeNeighborhoodsFromStructure(seed, state, clusterStructure, indexes);
    const r = encodeWithSeed(seed, state, params, predictions, nhs, indexes);
    const c = scoreCohesion(r.positions, state, params, indexes.commentPairs);
  ...
  members.forEach((m, id) => {
    m.mass = computeMass(id, state, params, indexes);
