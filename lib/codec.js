...
export const DEFAULT_PARAMS = {
  ...
  pairGravityScale: 3.0,     // multiplier on log-weight in scoreCohesion — strong pull between commenting pairs
};
...
export function encodeWithSeed(seed, state, params = DEFAULT_PARAMS, predictions = null) {
  ...
  // Collect join dates and engagement scores across all members.
  const refTime = state.epochDate ? new Date(state.epochDate).getTime() : Date.now();
  ...
  members.forEach((m, mid) => {
    ...
    const mass = computeMass(mid, state, params);
    if (mass > maxEngagement) maxEngagement = mass;
  });
  ...
  positions.forEach((p, mid) => {
    ...
    // XZ: engagement controls distance from center axis.
    // High engagement → small radius (core), low engagement → large radius (outskirts).
    const mass = computeMass(mid, state, params);
    const engT = Math.log(1 + mass) / Math.log(1 + maxEngagement); // 0=low, 1=high
...
export function scoreCohesion(positions, state, params = DEFAULT_PARAMS) {
  ...
  // Weighted mean distance: pairs with more shared comments contribute more.
  // Log-scale weight prevents high-volume pairs from collapsing the whole layout.
  let t = 0, n = 0;
  for (const k of Object.keys(pairs)) {
    const [a, b] = k.split('-');
    const pa = positions.get(a), pb = positions.get(b);
    if (pa && pb) {
      const w = Math.log(1 + pairs[k]) * (params.pairGravityScale ?? 1.0);
      t += v3dist(pa, pb) * w;
      n += w;
    }
  }
...
export function evolve(state, params = DEFAULT_PARAMS, predictions = null) {
  ...
  for (let i = 0; i < vc; i++) {
    ...
    const seed = i === 0 ? base : hash32(base + ':mut:' + Math.floor(Math.random() * Math.max(1, ms)) + ':' + Math.random());
