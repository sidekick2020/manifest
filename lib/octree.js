...
export class Octree {
  ...
  queryRadius(center, radius) {
    ...
    if (this.subdivided) {
      for (const child of this.children) {
        found.push(...child.queryRadius(center, radius));
      }
    }
 ...
 * Simplified: uses sphere around camera focus
 */
export function getFrustumBounds(camera, screenDimensions) {
  // For now, use a sphere around camera focus
  // TODO: Implement proper frustum plane extraction
  // Use a generous radius to avoid culling visible members
  const radius = camera.d * 3; // Increased multiplier for better coverage

  return {
    min: vec3(
      camera.focus.x - radius,
      camera.focus.y - radius,
      camera.focus.z - radius
    ),
    max: vec3(
      camera.focus.x + radius,
      camera.focus.y + radius,
